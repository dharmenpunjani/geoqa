...
public class Agdistis extends QanaryComponent {
	...
	public QanaryMessage process(QanaryMessage myQanaryMessage) throws Exception {
		try {
			...
			String input = lemmatize(myQuestion);
			Integer offset = 0;
			for (Spot spot : spots) {
			...
			for (int i = 0; i < arr.length(); i++) {
				if (!arr.getJSONObject(i).isNull("disambiguatedURL")) {
					...
					String yagoSparql = "select  ?x " + 
							...
							"        UNION\n" + 
							"        { ?x <http://yago-knowledge.org/resource/hasWikipediaUrl> <https://en.wikipedia.org/wiki/"+l.link.substring(l.link.lastIndexOf('/')+1)+"> . } " + 
							"      } "; 
...
//					continue;
//				String sparqlQuery = " select ?instance where { ?instance <http://www.app-lab.eu/osm/ontology#has_name> \""+instance+"\"^^<http://www.w3.org/2001/XMLSchema#string> . }";//?instance ?p ?o . }";
//				
//				String host = "pyravlos2.di.uoa.gr";
//				Integer port = 8080;
...
//						links.add(l);
//						System.out.println("Question: "+myQuestion+" ::: "+resultString[i].trim() + "== index : "+l.begin + " : "+l.end);
//						}
//						
//						System.out.println("<----- Result ----->");
//		
...
//					}
//				}
//				sparqlQuery = "select ?instance where { ?instance <http://www.app-lab.eu/gadm/ontology/hasName> \""+instance+"\"@en . }";//?instance ?p ?o . }";
//				query = sparqlQuery;
//				if (query.length() > 2) {
//					try {
//		
